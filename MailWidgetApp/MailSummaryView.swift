...
struct MailSummaryContentView: View {
    ...
    /// ⚙️/↻ 图标按钮组：与日报详情窗口的 header 统一样式（`WindowHeaderControls.swift`）。
    /// 原来这里是「↻ 重新总结」+「⏰ 自动化」两个带文字的按钮；用户对比两个窗口的
    /// 截图反馈长得不一样，统一成"左 ⚙️（图标 only）→ 右 ↻（图标 only）"——⚙️ 打开
    /// MailWidget 设置窗口，「自动化」整个搬到 `SettingsView`（见
    ...
    /// 生成中态用 spinner 替换图标（既有逻辑保留）；`.help(...)` 在两种状态下都给出
    /// 无障碍语义，因为按钮现在是纯图标、没有文字标签可读。
    @ViewBuilder
...
///
/// 挂载点是 `SettingsView.swift` 里的 `MailSummaryAutomationConfigButton`（原来挂在
/// 这个窗口 header 的 ⏰ 图标上，用户反馈两个 host 窗口的 header 该统一样式后，整个
/// 自动化入口搬去了 SettingsView——这个类型定义留在这里没动，只是调用方换了）。
