...
**2026-07-30 用户决定，覆盖本节约束**：日报卡片下方的「原始邮件行」（发件人 + 原始主题 +
相对时间）**已删除**，widget 与详情窗口两处同删。原因：日报覆盖的邮件大量来自 Canvas /
Instructure 这类通知网关，发件人显示名恒为某个人名、主题恒为模板套话（"...just sent you a
message in Canvas."），真实内容只在正文里；与 AI 写的中文标题并排时，这一行看起来像挂错了
邮件。已用 Gmail `rfc822msgid:` 反查实证：关联本身正确（Message-ID 精确指向被总结的那封
信），问题纯在于原始主题无信息量。删除后：未读蓝点移到 AI 标题行首（`isUnread` 语义不变），
整块卡片仍可点、落点链（那封信 → 该账户收件箱 → gmailURL）不变。
